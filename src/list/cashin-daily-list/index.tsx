  ...
  return (
    <tbody className="text-[13px]">
      {entries?.map((entry: any, mainIndex: number) => (
        <tr
          className={` h-[70px] capitalize text-left border-b`}
          key={mainIndex}
        >
          <td>{entry.no}</td>
          <td className="min-w-[160px]">{moment(entry.transactionDate).format('lll')}</td>
          <td className="min-w-[150px]">{entry.transactionId}</td>
          ...
          {/* <td>
            <Button
              onClick={() => navigate("/dashboard/cash-in-order/daily/detail")}
              variant="outlined"
...
const CashInDailyListTable = ({ loading, data }: any) => {
  const columns = [
    "no",
    "Date",
    "Transaction Id",
...
export default CashInDailyListTable;
