      ...
      {entries?.map((entry: any, mainIndex: number) => (
        ...
        >
          ...
          <td className="min-w-[160px]">
            {moment(entry.transactionDate).format("lll")}
          </td>
          <td className="min-w-[160px]">
              {entry.transactionId}
          </td>
          <td className="min-w-[150px]">
            {entry.fullname || "Delete user account"}
          </td>
          ...
          {/* <td>
            <Button
              onClick={() =>
                navigate("/dashboard/cash-out-order/yearly/detail")
              }
              variant="outlined"
              className="capitalize  button-item font-[400]  "
              color="blue"
              size="sm"
              placeholder={"button"}
            >
...
const CashOutYearListTable = ({ loading, data }: any) => {
  const columns = [
    "no",
    "Date",
    "Transaction Id",
  ...
  return (
    <div
      ref={ref}
      className="w-full bg-white p-5 px-5 pt-5 overflow-x-scroll  pb-10 relative min-h-[300px]  rounded-md shadow-sm  cursor-pointer"
    >
...
export default CashOutYearListTable;
