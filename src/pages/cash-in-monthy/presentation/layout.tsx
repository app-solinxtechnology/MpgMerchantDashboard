import {  buyMonthly } from "#Services";
import { useQuery, useMountOnce, useSkip } from "$myhooks";
import { useRequest } from "ahooks";
import moment from "moment";
import { useNavigate, useLocation } from "react-router-dom";
import CashInMonthlyList from "./CashInMonthyList"
...
const Layout = () => {
  ...
  const { loading, data, runAsync } = useRequest(buyMonthly,{
        manual :true
      }
  )
   useMountOnce(()=>{
  ...
  return  (
    <div  className="px-6 pt-6">
            <CashInMonthlyList  loading={loading} data={data}/>
    </div>
  )
}
